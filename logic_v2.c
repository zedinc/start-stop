...
#define THROTTLE_OPEN_DURATION 2000
#define THROTTLE_CLOSED_DURATION 4000
#define REVIVE_DURATION 3000
#define KILL_DURATION 1000
#define REVIVE_STATE "REVV"
#define KILL_STATE "KILL_"
#define ON_STATE "ON__"
#define OFF_STATE "OFF_"
...
void loop() {
  ...
  bool startTimeElapsed = timeElapsed(startTimer) >= REVIVE_DURATION ;
  ...
  else if ( prev_state == REVIVE_STATE && cdi == HIGH && startTimeElapsed ) {
	  state = ON_STATE;
  }
  
  else if ( prev_state == REVIVE_STATE && cdi == LOW && startTimeElapsed ) {
	  state = OFF_STATE;
  }
  ...
  else if ( state == REVIVE_STATE ) {
	  digitalWrite( REVIVE, HIGH );
	  digitalWrite( KILL, LOW );
  }
